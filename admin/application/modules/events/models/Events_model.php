...
class Events_model extends MY_Model {
    ...
    public function show() {
        $column = array('id', 'title', 'category', 'author', 'start_date', 'end_date', 'photo', 'description', 'account_name', 'account_no', 'location');
    ...
    public function count_total_row_of_event() {
    ...
    public function get_event_categories() {
        $query    = $this->db->where('is_active', 1)->get('event_categories');
        $category = array();
        if ($query->result()) {
